...
func TestCommandSurface(t *testing.T) {
	...
	for _, command := range []string{"bootstrap", "setup", "tunnel", "selective", "direct", "route", "vpn", "geoip", "status", "report", "logs", "info", "qr", "update", "restore-network", "uninstall", "direct-add", "direct-remove", "direct-list", "direct-edit", "proxy-add", "proxy-remove", "proxy-list", "proxy-edit", "ru", "foreign", "wifi"} {
		if _, _, err := root.Find([]string{command}); err != nil {
			t.Fatalf("command %s not found: %v", command, err)
		}
	}
	for _, args := range [][]string{{"vpn", "via-ru"}, {"vpn", "direct-auto"}, {"vpn", "direct"}} {
